...
export class ParticipateFormComponent {
  ...
  onSubmit() {
    console.log(this.participationForm.value); // Log the form values

    const toBeSent = {
      eventId: this.eventId,
      userId: this.userId,
      registrationDate: new Date().toISOString(),
      registrationCode: new Date().toISOString().toString(),
    ...
    this.s3 = this.participantService.postParticipant(toBeSent).subscribe((data) => {
      console.log(data);
    });
  }
